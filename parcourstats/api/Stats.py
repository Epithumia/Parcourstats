...
from parcourstats.utils import get_data, get_admissions, get_repartition
...
@resource(collection_path='/api/repartition', path='/api/repartition/{groupe}', factory=acl.RESTFactory)
class ApiRepartition(object):
    def __init__(self, request, context=None):
        self.request = request
    ...
    def collection_get(self):
        ...
        for f in formations:
            ...
            liste_f[f.Code] = (f.Nom, liste_groupes)
            for g in groupes:
                liste_repartition.append({g.code: get_repartition(g.code, dbsession)})
            if len(groupes):
                liste_repartition.append({-1: get_repartition('*', dbsession)})
            liste_stats[f.Code] = liste_repartition
